...
| `watchdog.md` | `requirements-elicitation.md` | Requirements for the RFC 5997/3539 connection-liveness watchdog on a RadSec session: `watchdog-interval` validation, the automatic send folded into `radcli_ctx_dispatch()`, `radcli_ctx_get_poll()`'s watchdog-deadline advisory, and `rc_check_tls()`'s opt-in wrapper (no public, caller-invoked send function). Split out of `config.md`/`dae.md`/`net.md` on 2026-08-30 to gather this cross-cutting concern into one place; every requirement carries `DERIVED`. |
...
| `watchdog.md` | `REQ-WATCHDOG` | RFC 5997; RFC 3539 §3.4; draft-ietf-radext-reverse-coa-08 §4.2; `lib/config.c`, `lib/dae.c` (`radcli2_priv_dae_send_watchdog()`, called automatically from `radcli_ctx_dispatch()`; `radcli_ctx_get_poll()`), `lib/tls.c` (`rc_check_tls()`); `tests/dae-radsec-watchdog.c`; `tests/watchdog-aaa.c` |
