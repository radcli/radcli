#!/usr/bin/perl -w
# Generates HEADER.h.3 from a public header and doxygen XML files. Used for
# both radcli.h (rc_* symbols) and radcli2.h (radcli_* symbols).
...
#
# Usage: gen-radcli-h-man.pl PUBLIC_HEADER MAP_FILE FILE_XML GROUP_XML [GROUP_XML ...]
#
# Only functions present in both PUBLIC_HEADER and MAP_FILE are included.
# rc_mksid and rc_setdebug are excluded from the overview (they get their own
# pages via the radcli_8h.xml doxy2man pass); harmless no-op for radcli2.h,
...
my $header    = shift @ARGV or die "Usage: $0 PUBLIC_HEADER MAP_FILE FILE_XML GROUP_XML...\n";
my $map_file  = shift @ARGV or die "Usage: $0 PUBLIC_HEADER MAP_FILE FILE_XML GROUP_XML...\n";
my $file_xml  = shift @ARGV or die "Usage: $0 PUBLIC_HEADER MAP_FILE FILE_XML GROUP_XML...\n";
my @group_xmls = @ARGV      or die "Usage: $0 PUBLIC_HEADER MAP_FILE FILE_XML GROUP_XML...\n";

my $xml_dir = dirname($file_xml);
my $header_base = basename($header);
my $include_path = "radcli/$header_base";

# Functions intentionally omitted from the summary page.
my %exclude = map { $_ => 1 } qw(rc_mksid rc_setdebug);

# --- Build export allowlist: intersection of PUBLIC_HEADER and MAP_FILE ---
...
my %in_header;
while ($header_text =~ /\b((?:rc_|radcli_)[a-z_0-9]+)\s*\(/g) {
    $in_header{$1} = 1;
}
...
print ".TH $header_base 3 $date \"radcli\" \"Radius client library\"\n";
print ".SH \"NAME\"\n";
print "$header_base \\- \n";
print ".SH SYNOPSIS\n";
print ".nf\n";
print ".B #include <$include_path>\n";
print ".fi\n";
