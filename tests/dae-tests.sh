...
echo " 8. A sweep of truncated packets (0, 1, 4, 19, 20, 25 bytes) is silently discarded"
echo " 9. A sweep of wire-Length lies (-10, +1, +4096 bytes) is silently discarded"
...
# Test 8/9: bounds/length handling (REQ-DAE-SEC-007) -- a sweep of truncation
# points and wire-Length lies, all of which must be silently discarded
# without reaching the handler, exercising process_packet()'s own header
# bounds-check (AUTH_HDR_LEN, "wire_length > length") and, via
# radcli_avp_decode()'s pkt_buf/pb_pull() (lib/avp.c), the attribute
# parser's own overflow returns. IDs 80-89 stay clear of every other test
# in this file so a wrongly-accepted packet here can't be masked as some
# other test's retransmission.
id=80
for n in 0 1 4 19 20 25; do
	run_test "A packet truncated to $n bytes is silently discarded" \
		"client --id $id --truncate $n | grep -q '^NO-REPLY'" \
		|| exit 1
	id=$((id + 1))
done
for delta in -10 1 4096; do
	run_test "A packet whose wire Length field lies by $delta bytes is discarded" \
		"client --id $id --bad-length $delta | grep -q '^NO-REPLY'" \
		|| exit 1
	id=$((id + 1))
done
