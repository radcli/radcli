...
def build_packet(code, ident, secret, attrs, authenticator_mode, msg_auth_mode, bad_length_delta):
    ...
    if bad_length_delta:
        wire_len += bad_length_delta  # the wire Length field lies about how much follows
...
def main():
    ...
    parser.add_argument('--bad-length', type=int, nargs='?', const=4096, default=None,
                        metavar='DELTA',
                        help='the wire Length field lies by DELTA bytes relative to what '
                             'was actually sent (default 4096 if given with no value; '
                             'negative understates it)')
    ...
    packet = build_packet(code, args.id, args.secret, attrs, args.authenticator,
                          args.msg_auth, args.bad_length or 0)
