...
int main(int argc, char** argv)
{
    ...
    char        username[255]       = "test";
    char        password[255]       = "test";
    ...
    if (rc_avpair_add(rh, &send, PW_CHAP_PASSWORD, password, -1, 0) == NULL)
    {
        printf("ERROR: Failed adding Chap-passwordUser-Name: to %s\n", password);
        exit(1);
    }
    ...
    {
        printf("INFO: Authentication OK: %s\n", username);
    }
    else
    {
        printf("INFO: Authentication Failed: %s\n", username);
        retval = -1;
    }
