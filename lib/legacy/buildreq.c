...
/*- Select the server list and rc_type for a request, based on transport
 * and request type. Internal helper shared by rc_aaa_ctx() and
 * rc_acct_async().
 *
 * @param rh a handle to parsed configuration.
...
/*- Fill in NAS-Port and Acct-Delay-Time on a request being built.
 * Internal helper shared by rc_aaa_ctx_server() and
 * rc_aaa_ctx_server_async().
 *
 * @param rh a handle to parsed configuration.
 * @param data the request being built; send_pairs is extended in place.
 ...
 * @param request_type one of the standard RADIUS codes (e.g., PW_ACCESS_REQUEST).
 * @param adt_vp receives the Acct-Delay-Time pair when request_type is
		      ...
		      rc_standard_codes request_type)
{
	...
	int result;
	int timeout = rc_conf_int_id(rh, OPT_RADIUS_TIMEOUT);
	int retries = rc_conf_int_id(rh, OPT_RADIUS_RETRIES);
	double start_time = 0;
	time_t dtime;
	int servernum;
	...
	do {
		...
		DEBUG(rh, LOG_INFO, "rc_send_server_ctx returned error (%d) for server %u: (remaining: %d)",
...
/*- Send an accounting request to every server in aaaserver without
 * waiting for a reply. Internal helper behind rc_acct_async().
 *
 * @param rh a handle to parsed configuration.
 * @param aaaserver a non-NULL SERVER describing the target server(s).
