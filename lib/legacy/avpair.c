...
VALUE_PAIR *rc_avpair_new (rc_handle const *rh, uint32_t attrid, void const *pval, int len, uint32_t vendorspec)
{
	...
	if ((vp = malloc (sizeof (VALUE_PAIR))) != NULL)
	{
		...
		vp->next = NULL;
		vp->type = radcli_dict_type_to_legacy(pda->type);
...
/*- Decode a run of RADIUS attribute TLVs from pb, prepending onto pair.
 *
 * @param rh a handle to parsed configuration.
 * @param pair existing VALUE_PAIR list to prepend the decoded attributes
 * onto; freed on hard error.
 * @param pb the buffer to decode attributes from.
 * @param vendorspec 0 when decoding top-level attributes, or the
 * enclosing vendor's PEN when recursing into a VSA's sub-attributes.
 * @param out set to the decoded list on success (may be NULL if every
 * attribute was skipped).
 * @return 0 on success, -1 on a hard error (*out undefined, pair freed).
 -*/
			  ...
			  VALUE_PAIR **out)
{
	...
	while (pb_len(pb) > 0) {
		...
		rpair->type = radcli_dict_type_to_legacy(attr->type);
...
/*- Copy a data field from the buffer, advancing *uptr past it. No more
 * than len - 1 bytes are copied, and string is always NUL-terminated.
 *
 * @param string the destination buffer to copy into.
 * @param uptr the current position of the buffer; advanced past the field.
 * @param stopat characters at which parsing should stop.
...
int rc_avpair_parse (rc_handle const *rh, char const *buffer, VALUE_PAIR **first_pair)
{
	...
	while (*buffer != '\n' && *buffer != '\0')
	{
		...
		switch (mode)
		{
			...
			pair->type = radcli_dict_type_to_legacy(attr->type);
